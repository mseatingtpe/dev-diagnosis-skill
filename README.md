...
flowchart TD
    ...
    SkillReview -->|清單是空的| ColdStart[跳過復盤 → 延展性評估]
    ...
    LevelOutput --> SkillQ{值得寫成 skill？}
    SkillQ -->|是| WriteSkill[建立 skill 結構]
    SkillQ -->|否| Register[登記到既有 skill 清單]
    WriteSkill --> Place{管理層級}
    Place -->|受管| Repo[進 GitHub repo]
    Place -->|輕量| RepoLight[進 GitHub repo + README]
    Place -->|自由| Personal[存個人空間]
    Repo --> Register
    RepoLight --> Register
    Personal --> Register
    Register --> NextStep[具體下一步行動]
